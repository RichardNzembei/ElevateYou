...
import { auth, db } from '@/firebase-client'; // Updated import
...
export default {
  ...
  setup() {
    ...
    // Fetch user data from Firestore only on the client side
    onMounted(() => {
      if (!process.client || !auth || !db) return; // Skip if not on client or Firebase isn't initialized
    ...
    // GitHub Connection for Developer Card
const connectGitHub = async () => {
  if (!process.client || !auth) {
    console.error('Cannot connect GitHub: Not running on client or auth not initialized');
    return;
  }
  ...
  try {
    ...
    await setDoc(doc(db, 'Users', userId), { devTracker }, { merge: true });
  } catch (error) {
    ...
    // Google Fit Connection for Exercise Card
    const connectGoogleFit = async () => {
      ...
      try {
        ...
        await setDoc(doc(db, 'Users', userId), { workouts }, { merge: true });
      } catch (error) {
    ...
    const requestScreenTimePermission = async () => {
      ...
      await setDoc(doc(db, 'Users', userId), { screenTime }, { merge: true });
    };
